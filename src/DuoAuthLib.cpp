...
void DuoAuthLib::begin(const char* duoApiHost, const char* duoApiIKey, const char* duoApiSKey, const char* rootCa, struct tm* timeInfo)
{
	_timeinfo = timeInfo;
...
bool DuoAuthLib::ping()
{
	...
	bool apiResponse = submitApiRequest(HTTP_GET, (char *)DUO_PING_API_PATH, "", "", "");
...
bool DuoAuthLib::checkApiKey()
{
	...
	if(getLocalTime(_timeinfo)){  
		...
		generateHmacPayload(hmacPayload, timeStringBuffer, "GET", _duoHost, DUO_CHECK_API_PATH, "");
		
		// Generate the required URL Request Contents for the DUO API Call
		calculateHmacSha1((char *)_duoSkey, hmacPayload, hmac_password);  
		
		bool apiResponse = submitApiRequest(HTTP_GET, timeStringBuffer, DUO_CHECK_API_PATH, hmac_password, "");
...
{	
	...
	if(transactionId.length() == 36){
		...
		if(getLocalTime(_timeinfo)){  
			...
			// Generate the required URL Request Contents for the DUO API Call
			generateHmacPayload(hmacPayload, timeStringBuffer, "GET", _duoHost, DUO_AUTHSTATUS_API_PATH, authRequestContents);

			// Generate the required URL Request Contents for the DUO API Call
			calculateHmacSha1((char *)_duoSkey, hmacPayload, hmac_password);  
			
			bool apiResponse = submitApiRequest(HTTP_GET, timeStringBuffer, DUO_AUTHSTATUS_API_PATH, hmac_password, authRequestContents);
...
bool DuoAuthLib::pushAuth(char* userName, bool async)
{
	...
	return performAuth(encodedUsername, "", PUSH);
}
...
bool DuoAuthLib::performAuth(char* userName, char* userPasscode, enum DUO_AUTH_METHOD authMethod)
{
	...
	if(getLocalTime(_timeinfo)){  
		...
		if(authMethod == PUSH){
			if(_async){
				addRequestParameter(authRequestContents, (char *)ASYNC_PARAM, "1");
			}
		...
		// Generate the required URL Request Contents for the DUO API Call
		generateHmacPayload(hmacPayload, timeStringBuffer, "POST", _duoHost, DUO_AUTH_API_PATH, authRequestContents);
				
		// Generate the required URL Request Contents for the DUO API Call
		calculateHmacSha1((char *)_duoSkey, hmacPayload, hmac_password);  
		
		bool apiResponse = submitApiRequest(HTTP_POST, timeStringBuffer, DUO_AUTH_API_PATH, hmac_password, authRequestContents);
...
bool DuoAuthLib::submitApiRequest(enum DUO_HTTP_METHOD apiMethod, char *timeString, const char* apiPath, char *hmacPassword, char* requestContents)
{
	...
	if(apiMethod == HTTP_GET && strlen(requestContents) > 0){
		requestUrl += '?';
		requestUrl += requestContents;
	}
	
	http.begin(requestUrl, _rootCa); //Specify the URL and certificate
